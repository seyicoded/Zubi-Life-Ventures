            ...
            @foreach ($data['db'] as $dt)
                <tr>
                    ...
                    <td>
                        @switch(intval($dt->status))
                            @case(0)
                            <button type="button" class="btn btn-danger" onclick="window.location.href= '{{url('admin/cancel-subscription')}}?trxref={{$dt->tnx_ref}}'">Cancel Subscription</button>
                                @break

                            @case(1)
                            <button type="button" class="btn btn-danger" onclick="window.location.href= '{{url('admin/cancel-subscription')}}?tnx_ref={{$dt->tnx_ref}}'">Cancel Subscription</button>
                                @break
