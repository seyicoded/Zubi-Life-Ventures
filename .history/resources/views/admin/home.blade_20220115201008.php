<?php
    ...
    $num_worker = count(DB::select('SELECT * from admins where a_status = ?', [1]));
    $num_doctor = count(DB::select('SELECT * from universal_account where account_type = ?', ['doctor']));
    $num_product = count(DB::select('SELECT * from drugs where status = ?', [1]));

    $recent_product = DB::select('SELECT * from drugs where status = ? ORDER BY d_id DESC LIMIT 3', [1]);
?>
